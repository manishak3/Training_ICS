...
class Program
{
    static void Main()
    {
        ...
        // Populate empList with provided records


        // LINQ Queries
        Console.WriteLine("a. Display detail of all the employees:");
        var allEmployees = empList.Select(emp => $"{emp.EmployeeID} - {emp.FirstName} {emp.LastName}, {emp.Title}, {emp.City}");
        ...
        var employeesNotInMumbai = empList.Where(emp => emp.City != "Mumbai").Select(emp => $"{emp.EmployeeID} - {emp.FirstName} {emp.LastName}, {emp.Title}, {emp.City}");
        ...
        var asstManagers = empList.Where(emp => emp.Title == "AsstManager").Select(emp => $"{emp.EmployeeID} - {emp.FirstName} {emp.LastName}, {emp.Title}, {emp.City}");
        ...
        var employeesWithLastNameStartingWithS = empList.Where(emp => emp.LastName.StartsWith("S")).Select(emp => $"{emp.EmployeeID} - {emp.FirstName} {emp.LastName}, {emp.Title}, {emp.City}");
